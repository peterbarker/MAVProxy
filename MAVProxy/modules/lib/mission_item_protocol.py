...
class MissionItemProtocolModule(mp_module.MPModule):
    ...
    def send_wp_requests(self, wps=None):
        ...
        for seq in wps:
            ...
            if self.settings.wp_use_mission_int:
                self.master.mav.mission_request_send(self.target_system, self.target_component, seq, mission_type=self.mav_mission_type())
            else:
                self.master.waypoint_request_send(self.target_system, self.target_component, seq)
    ...
    def mavlink_packet(self, m):
        ...
        if mtype in ['WAYPOINT_COUNT','MISSION_COUNT']:
            ...
            if m.seq+1 > self.wploader.expected_count:
                self.console.writeln("Unexpected %s number %u - expected %u" % (
                    self.itemtype(),
                    m.seq,
                    self.wploader.count()))
    ...
    def cmd_wp(self, cmdname, args):
        '''waypoint commands'''
        usage = "usage: %s <editor|list|load|update|save|set|clear|loop|remove|move|movemulti|changealt>" % cmdname
        if len(args) < 1:
            print(usage)
            return
        ...
        if args[0] == "load":
            if len(args) != 2:
                print("usage: %s load <filename>" % cmdname)
                return
...
#                f.write("# %s\n" % w.comment)
    ...
    def fetch(self):
        ...
        if self.wp_op is None:  # If we were already doing a list or save, just restart the fetch without changing the operation
            self.wp_op = "fetch"
        if self.master.mavlink20():
            self.master.mav.mission_requst_list_send(self.target_system,
                                                     self.target_component,
                                                     mission_type=self.mav_mission_type())
        else:
            self.master.waypoint_request_list_send()
